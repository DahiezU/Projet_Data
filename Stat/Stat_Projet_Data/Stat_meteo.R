...
# On observe qu'on   rejette l'hypoth√®se H0  car la p-value est > 0.01 , donc il y une correlation
...
cor.test( winter$light, winter$rain, method=c("pearson", "kendall", "spearman"))  #-0.02739696  --> la correlation est moins prononc√© que pour les autes saisons 
...
cor.test( spring$light, spring$rain, method=c("pearson", "kendall", "spearman")) #-0.09661923 --> plus il pleut moins il y a de lumi√®re 



cor.test( winter$temperature, winter$rain, method=c("pearson", "kendall", "spearman"))  #-0.08051924  --> la correlation est moins prononc√© que pour les autes saisons
...
cor.test( winter$pressure, winter$rain, method=c("pearson", "kendall", "spearman"))  #-0.02696862  --> la correlation est moins prononc√© que pour les autes saisons
...
cor.test( winter$humidity, winter$rain, method=c("pearson", "kendall", "spearman"))  #0.4274842   --> la correlation est moins prononc√© que pour les autes saisons
cor.test( summer$humidity, summer$rain, method=c("pearson", "kendall", "spearman")) #0.22722  --> Plus il pleut, plus l'humidit√© monte 
cor.test( autumn$humidity, autumn$rain, method=c("pearson", "kendall", "spearman")) #0.2617195    --> Plus il pleut, plus l'humidit√© monte 
cor.test( spring$humidity, spring$rain, method=c("pearson", "kendall", "spearman")) #0.3184181 --> Plus il pleut, plus l'humidit√© monte 
...
# 
#   plot( elem[" humidity"], main="HumiditÈ au fil des heures entre 2 perdiodes de pluie",
#         ylab="% HumiditÈ",
#         xlab="Index DonnÈes",
#         type="l",
#   col="blue")
